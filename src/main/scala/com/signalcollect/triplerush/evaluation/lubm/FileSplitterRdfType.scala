...
object FileSplitterRdfType extends App {

  def assemblyPath = "./target/scala-2.10/triplerush-assembly-1.0-SNAPSHOT-RDFType.jar"
  val kraken = new TorqueHost(
    jobSubmitter = new TorqueJobSubmitter(username = System.getProperty("user.name"), hostname = "kraken.ifi.uzh.ch"),
    coresPerNode = 1,
    localJarPath = assemblyPath, priority = TorquePriority.slow)
  ...
  def splitNtriples(mod: Int, baseSourceFolderName: String)(): List[Map[String, String]] = {
    ...
    val parallelism = 4
    val sourceFolder = s"./lubm$baseSourceFolderName-type-binary"
