...
import com.signalcollect.triplerush.evaluation.QueryEncodingRdfType

object DictionaryEncoderWithBaseBDBRdfType extends KrakenExecutableRdfType with Serializable {
  run(BaseEncoderBDBRdfType.encode(args(0)) _)
}

case object BaseEncoderBDBRdfType extends Serializable {

  def encode(sourceFolderBaseName: String)() {
    import FileOperationsType._

    val sourceFolderName = s"./${sourceFolderBaseName}-type-nt"
    ...
    for (entry <- QueryEncodingRdfType.m) {
    ...
    def encodeFile(source: File, target: File) {
      ...
      while (nxp.hasNext) {
        ...
        if (!subjectString.startsWith("file:///Users")) {
          ...
          val predicateStringRdfType = (if (predicateString == QueryEncodingRdfType.rdfTypeStr) predicateString + objectString else predicateString)

          val sId = register(subjectString)
          val pId = register(predicateStringRdfType)
