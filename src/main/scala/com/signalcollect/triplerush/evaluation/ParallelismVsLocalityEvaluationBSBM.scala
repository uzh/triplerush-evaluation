...
class ParallelismVsLocalityEvaluationBSBM extends TorqueDeployableAlgorithm {
      ...
      for (subQueryId <- listOfSubQueryIds) {
        if (queryRun <= 11) {
          ...
          if (patterns.size == 1 && firstPatternSequence.size >= 3) {
            ...
            println(s"Running evaluation for query $queryId-$subQueryId queryrun: $queryRun. Awaiting idle")
