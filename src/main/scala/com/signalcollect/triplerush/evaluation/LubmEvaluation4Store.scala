...
class LubmEvaluation4Store extends TorqueDeployableAlgorithm {
  ...
  def execute(parameters: Map[String, String], nodeActors: Array[ActorRef]) {
    ...
    val dbName = s"lubm${datasetSize}nn$numberOfNodes"
    ...
    val clusterBsbmImportCommand = s"4s-import $dbName --format ntriples $bsbmNtriplesFileLocation"
    //using concatenated ntriples fie.
    val clusterLubmImportCommand = s"4s-import $dbName -v --format ntriples lubm-nt-files/lubm$datasetSize.nt"
    
    val importStartTime = System.nanoTime()
    val clusterBsbmImport = (clusterBsbmImportCommand).!
    val clusterLubmImport = (clusterLubmImportCommand).!
    val importEndTime = System.nanoTime()
    val loadingTime = roundToMillisecondFraction(importEndTime - importStartTime)
    
    println(s"Loaded lubm files: $clusterLubmImport")
    println(s"Finished loading BSBM: $clusterBsbmImport")
    Thread.sleep(60000)
    ...
    val bsbmQueriesObject = Class.forName(bsbmQueriesObjectName).newInstance.asInstanceOf[BerlinSparqlQueries]
    val bsbmQueries = bsbmQueriesObject.queries
    ...
    def warmup {
      ...
      if (warmupRuns != 0) {
        for (i <- 1 to (warmupRuns / 20)) {
          for ((queryId, listOfSubQueryIds) <- bsbmQueriesObject.warmupQueries) {
            val listOfQueries = bsbmQueries(queryId)
    ...
    for (queryId <- lubmQueries.size to 1 by -1) {
      val query = lubmQueries(queryId - 1)
