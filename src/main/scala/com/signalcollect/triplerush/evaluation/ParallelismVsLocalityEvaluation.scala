...
import com.signalcollect.triplerush.SingleNodeTripleMapper
import com.signalcollect.triplerush.LoadBalancingTripleMapper
import com.signalcollect.triplerush.AlternativeTripleMapperFactory
import com.signalcollect.triplerush.SingleNodeTripleMapperFactory
import com.signalcollect.triplerush.LoadBalancingTripleMapperFactory
import com.signalcollect.triplerush.AlternativeTripleMapperFactory

class ParallelismVsLocalityEvaluation extends TorqueDeployableAlgorithm {
  ...
  def execute(parameters: Map[String, String], nodeActors: Array[ActorRef]) {
    ...
    val tripleMapperObjectName = parameters("tripleMapperFactory")

    val tripleMapperFactory = {
      tripleMapperObjectName match {
        case "com.signalcollect.triplerush.SingleNodeTripleMapperFactory" =>
          SingleNodeTripleMapperFactory
        case "com.signalcollect.triplerush.LoadBalancingTripleMapperFactory" =>
          LoadBalancingTripleMapperFactory
        case "com.signalcollect.triplerush.AlternativeTripleMapperFactory" =>
          new AlternativeTripleMapperFactory(false)
      }
    }

    //val tripleMapperFactory = Class.forName(tripleMapperObjectName).newInstance.asInstanceOf[MapperFactory[Long]]
    ...
    println(s"Starting TR with tripleMapperFactory: ${tripleMapperFactory.toString()}")
    val graphBuilder = new GraphBuilder[Long, Any]().withPreallocatedNodes(nodeActors)
    val tr = new TripleRush(graphBuilder, optimizerCreator = optimizerCreator, tripleMapperFactory = Some(tripleMapperFactory))
    ...
    commonResults += s"tripleMapperFactory" -> tripleMapperFactory.toString()
    ...
    def warmupForXMs(patterns: Seq[TriplePattern], selectVariables: Int, timeOut: Int) {
      ...
      while (secondsElapsed < timeOut) {
        ...
        val result = executeEvaluationRun(patterns, selectVariables, 0, "0", tr, optimizer, commonResults)
    ...
    for (queryId <- lubmQueries.size to 1 by -1) {
      ...
      if (patterns.size == 1 && firstPatternSequence.size >= 3) {
        ...
        for (queryRun <- 1 to 11) {
          ...
          val result = executeEvaluationRun(optimizedQueryPlan, selectVariables, queryRun, queryId.toString, tr, optimizer, commonResults)
          println(s"Done running evaluation for query $queryId-$queryRun. Awaiting idle")
  ...
  def executeEvaluationRun(query: Seq[TriplePattern], selectVariables: Int, queryRun: Int, queryDescription: String, tr: TripleRush, optimizer: Optimizer, commonResults: Map[String, String]): Map[String, String] = {
    ...
    println(s"first result: total: $executionTime, numresults: $numberOfResults")
