...
class BerlinSparqlEvaluationTorque extends TorqueDeployableAlgorithm {
  ...
  def execute(parameters: Map[String, String], nodeActors: Array[ActorRef]) {
	...
	*/
    ...
    JvmWarmup.sleepUntilGcInactiveForXSeconds(60, 600)
    ...
    for ((queryId, queries) <- queries) {
      //var subQueryId = 1
      for (subQueryId <- 1 to 25) {
        //for (q <- queries) {
        val q = queries(subQueryId)
        println(s"Running evaluation for query $queryId-$subQueryId.")
        val result = executeEvaluationRun(q, subQueryId, s"${queryId.toString}-${subQueryId}", tr, commonResults)
        resultReporter(result)
        println(s"Done running evaluation for query $queryId-$subQueryId. Awaiting idle")
        JvmWarmup.sleepUntilGcInactiveForXSeconds(10)
        println("Idle")
  ...
  def executeEvaluationRun(queryString: String, queryRun: Int, queryDescription: String, tr: TripleRush, commonResults: Map[String, String]): Map[String, String] = {
    ...
    //println(s"query is: $queryString, tr is: ${tr.toString}")
    val queryOption = Sparql(queryString)(tr)
    //println(s"queryOption is: ${queryOption.toString}")
